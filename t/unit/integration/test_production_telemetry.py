...
from celery.app.validation import EnhancedOption, ConfigurationValidator, ValidationError
...
class TestConfigurationValidation:
    """Test enhanced configuration validation system."""
    
    def test_enhanced_option_type_conversion(self):
        """Test type conversion with comprehensive validation."""
        # Integer option with range validation
        int_option = EnhancedOption(
            default=10,
            type_=int,
            min_value=1,
            max_value=100
        ...
        # Valid conversion
        assert int_option.validate_and_convert("50", "test_option") == 50
        assert int_option.validate_and_convert(25, "test_option") == 25
        
        # Invalid range
        with pytest.raises(ValidationError):
            int_option.validate_and_convert(150, "test_option")
        
        with pytest.raises(ValidationError):
            int_option.validate_and_convert(-5, "test_option")
    
    def test_boolean_conversion_robustness(self):
        """Test robust boolean conversion from various formats."""
        bool_option = EnhancedOption(type_=bool)
        
        # String representations
        assert bool_option.validate_and_convert("true", "test") is True
        assert bool_option.validate_and_convert("TRUE", "test") is True
        assert bool_option.validate_and_convert("1", "test") is True
        assert bool_option.validate_and_convert("yes", "test") is True
        assert bool_option.validate_and_convert("on", "test") is True
        assert bool_option.validate_and_convert("enabled", "test") is True
        
        assert bool_option.validate_and_convert("false", "test") is False
        assert bool_option.validate_and_convert("0", "test") is False
        assert bool_option.validate_and_convert("no", "test") is False
        assert bool_option.validate_and_convert("disabled", "test") is False
    ...
    def test_url_validation_security(self):
        """Test URL validation with security constraints."""
        from celery.app.validation import validate_broker_url
        
        # Valid URLs
        assert validate_broker_url("redis://localhost:6379/0") == "redis://localhost:6379/0"
        assert validate_broker_url("amqp://user:pass@localhost:5672//") == "amqp://user:pass@localhost:5672//"
        
        # Invalid schemes
        with pytest.raises(ValueError, match="Unsupported broker scheme"):
            validate_broker_url("invalid://localhost:6379")
        
        # Empty URL
        with pytest.raises(ValueError, match="cannot be empty"):
            validate_broker_url("")
    
    def test_configuration_validator_integration(self):
        """Test complete configuration validation workflow."""
        validator = ConfigurationValidator()
        
        # Test configuration
        config = {
            "broker_connection_timeout": "10.0",  # String input
            "worker_concurrency": 8,              # Integer input  
            "task_serializer": "json",            # Valid choice
        }
        
        # Define options for validation
        options = {
            "broker_connection_timeout": EnhancedOption(
                type_=float, min_value=0.1, max_value=300.0
            ),
            "worker_concurrency": EnhancedOption(
                type_=int, min_value=1, max_value=1000
            ),
            "task_serializer": EnhancedOption(
                type_=str, choices=["json", "pickle", "yaml", "msgpack"]
            ),
        }
        
        # Validate configuration
        validated = validator.validate_configuration(config, options)
        
        # Verify type conversion and validation
        assert validated["broker_connection_timeout"] == 10.0
        assert validated["worker_concurrency"] == 8
        assert validated["task_serializer"] == "json"
        
        # Should have no validation errors
        assert len(validator.validation_errors) == 0
    
    def test_validation_error_reporting(self):
        """Test detailed error reporting for operations teams."""
        option = EnhancedOption(type_=int, min_value=10, max_value=100)
        
        try:
            option.validate_and_convert(5, "worker_concurrency")
        except ValidationError as e:
            assert "worker_concurrency" in str(e)
            assert "below minimum" in str(e)
            assert "10" in str(e)  # Threshold mentioned
            assert e.field_name == "worker_concurrency"
            assert e.value == 5
