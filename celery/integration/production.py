...
from celery.app.validation import validate_celery_configuration, ValidationError
...
def enable_production_telemetry(
    ...
    Args:
        ...
        config_validation_enabled: Enable enhanced configuration validation
    ...
    if config_validation_enabled:
        _integrate_config_validation(app)
...
def _integrate_config_validation(app) -> None:
    """Integrate enhanced configuration validation into app configuration."""
    
    # Store original configuration
    original_config = dict(app.conf)
    
    try:
        # Validate configuration with enhanced rules
        validated_config = validate_celery_configuration(original_config)
        
        # Update app configuration with validated values
        app.conf.update(validated_config)
        
        logger.info(
            "Configuration validation successful: %d options validated",
            len(validated_config)
        )
        
    except ValidationError as e:
        logger.error("Configuration validation failed: %s", e)
        # In production, you might want to fail fast or use defaults
        logger.warning("Continuing with original configuration due to validation errors")
